...
public class CombatAssistant{
	...
	public final double DEFENCE_MODIFIER = 0.70;
	...
	**/
		...
		if (Server.playerHandler.players[i] != null) {
			...
			c.followId = i;
			c.followId2 = 0;
			if (!Region.canAttack(c, (Client)Server.playerHandler.players[i])) {
				c.getPA().playerWalk(Server.playerHandler.players[i].getX(), Server.playerHandler.players[i].getY());
			...
			if(c.attackTimer <= 0) {
				...
				if(!usingBow && !c.usingMagic && !usingOtherRangeWeapons) { // melee hit delay
					...
					c.projectileStage = 0;
					c.oldPlayerIndex = i;
					c.curses().startSS((Client)Server.playerHandler.players[c.playerIndex], c.hitDelay);
				}
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				int damage = c.ramboModeInitiated ? ((int) ((calculateMeleeMaxHit() * .90) + Misc.random(100))) : Misc.random(rangeMaxHit());
				int damage2 = -1;
				if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1)
					damage2 = c.ramboModeInitiated ? ((int) ((calculateMeleeMaxHit() * .90) + Misc.random(100))) : Misc.random(rangeMaxHit());
				boolean ignoreDef = false;
				...
				if (damage > 0 && Misc.random(5) > 0 && c.lastArrowUsed == 9244) {
					damage *= BoltSpecials.boltMultiplier(o, 9244);
				}
	...
	public void applyPlayerMeleeDamage(int i, int damageMask){
		...
		if (!meleeHitSuccess(calculateMeleeAttack(), o.getCombat().calculateMeleeDefence()) && !veracsEffect) {
			...
			o.getPA().appendPoison(c);//This is disabled due to poison hitmarks working 192837
		...
		switch (c.specEffect) {
			...
			case 10:
				...
				if (c.hit1 > 0)
					...
					c.hit2 = meleeHitSuccess(calculateMeleeAttack(), o.getCombat().calculateMeleeDefence()) ? c.ramboModeInitiated ? ((int) ((calculateMeleeMaxHit() * .90) + Misc.random(100))) : Misc.random(calculateMeleeMaxHit()) : 0;
				if (c.hit2 > 0)
					c.hit3 = c.hit2/2;
				else
					c.hit3 = meleeHitSuccess(calculateMeleeAttack(), o.getCombat().calculateMeleeDefence()) ? c.ramboModeInitiated ? ((int) ((calculateMeleeMaxHit() * .90) + Misc.random(100))) : Misc.random(calculateMeleeMaxHit()) : 0;
				if (c.hit3 > 0)
					c.hit4 = c.hit3 + 1;
				else
					c.hit4 = meleeHitSuccess(calculateMeleeAttack(), o.getCombat().calculateMeleeDefence()) ? c.ramboModeInitiated ? ((int) ((calculateMeleeMaxHit() * .90) + Misc.random(100))) : Misc.random(calculateMeleeMaxHit()) : 1;
	...
	public void vineWhip(int index, int damage) {
		int[] effectedWhips = {15441, 15442, 15443, 15444};
		for(int i = 0; i < effectedWhips.length; i++) {
        if(c.playerEquipment[c.playerWeapon] == effectedWhips[i]) {
	...
	if(c.playerEquipment[c.playerRing] == 19669) {
		...
		switch(weapon) {
			...
			/*case 1249: 
                ...
                //break;
					Client a = (Client)Server.playerHandler.players[i];
					//o.getPA().getSpeared(c.absX, c.absY);
					//
					//Client a = (Client)attacked;
					c.lastSpear = System.currentTimeMillis();
				if(c.absX > a.absX) {
                if(Region.canMove(a.absX, a.absY, a.absX - 1, a.absY, a.heightLevel, 1, 1))
                    a.getPA().walkTo(0, -1);
                break;
				}
				if(c.absX < a.absX) {
            ...
            if(c.absY > a.absY) {
                if(Region.canMove(a.absX, a.absY, a.absX, a.absY - 1, a.heightLevel, 1, 1))
                    a.getPA().walkTo(-1, 0);
                break;
            }
            if(c.absY < a.absY && Region.canMove(a.absX, a.absY, a.absX, a.absY + 1, a.heightLevel, 1, 1))
                a.getPA().walkTo(1, -1);
            break;
			*/
	...
	**/
	...
	public void handlePrayerDrain() {
		...
		for (int i = 0; i < c.curses().CURSE_DRAIN.length; i++) {
			if (c.curseActive[i]) {
				toRemove += c.curses().CURSE_DRAIN[i]/10;
				c.usingPrayer = true;
			}
		}
	...
	**/
	public void addCombatXP(int attackType, int damage) {
		...
		switch (attackType) {
			...
			break;
			case 2: // Magic
				int magicXP = (Config.MAGIC_EXP_RATE * damage) + c.MAGIC_SPELLS[c.oldSpellId][7];
	...
	**/
	...
	public boolean checkReqs() {
		...
		if(!Server.playerHandler.players[c.playerIndex].inWild() && !c.playerName.toLowerCase().equals("xyle") && !c.playerName.toLowerCase().equals("ey cuzzo") && !c.playerName.toLowerCase().equals("situations") && !c.playerName.toLowerCase().equals("square")) {
			c.sendMessage("That player is not in the wilderness.");
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
		if(!c.inWild() && !c.playerName.toLowerCase().equals("xyle") && !c.playerName.toLowerCase().equals("ey cuzzo") && !c.playerName.toLowerCase().equals("situations") && !c.playerName.toLowerCase().equals("square")) {
			c.sendMessage("You are not in the wilderness.");
			c.stopMovement();
			c.getCombat().resetPlayerAttack();
			return false;
		}
		if(Config.COMBAT_LEVEL_DIFFERENCE && !c.playerName.toLowerCase().equals("xyle") && !c.playerName.toLowerCase().equals("ey cuzzo") && !c.playerName.toLowerCase().equals("situations") && !c.playerName.toLowerCase().equals("square")) {
	...
	**/
	...
	public int getWepAnim(String weaponName) {
		...
		if(weaponName.contains("light") || weaponName.contains("staff")) {
			switch(c.fightMode) {
				case 0:
				return 15071;	
				case 1:
	...
	*/
	public int getBlockEmote() {
		...
		if (weapon.contains("rapier") || weapon.contains("scimitar") || weapon.contains("korasi"))
			...
			return 13054;
                if (weapon.contains("2h sword") || weapon.contains("godsword") || weapon.contains("saradomin sword"))
			return 13051;
                if (weapon.contains("longsword"))
			return 13042;
                if (weapon.contains("greataxe"))
			return 12004;
                if (weapon.contains("flail"))
			return 2063;
                if (weapon.contains("hand cannon"))
			return 12156;
	...
	**/
	...
	String [] fast = {"dart", "knive"};
	for (int i = 0; i < fast.length; i++) {
		 if(weapon.contains(fast[i])){
			return fastSpeed;
			}
		}
			int standardSpeed = 0;
	...
	"ancient staff","chaotic rapier"};
	for (int j = 0; j < standard.length; j++){
		 if(weapon.contains(standard[j])){
			return standardSpeed;
			}
		}
	String [] slow = {"longsword","mace","hatchet","spear","pickaxe","tzhaar-ket-em","torag's hammers","guthan's warspear",
                          "verac's flail","staff of light","throwing axe","crystal bow","ogre composite bow", "seercull"};
                    int slowSpeed = 5;
	for (int k = 0; k < slow.length; k++) {
		if (weapon.contains(slow[k])){
			return slowSpeed;
			}
		}
		
	String [] verySlow = {"battleaxe","warhammer","godsword","barrelchest anchor","ahrim's staff","toktz-mej-tal","chaotic maul",
                              "gravite_2h_sword","longbow","god bow","javelin","crossbow","hand cannon"};
			int verySlowSpeed = 0;
			verySlowSpeed += (double) 6.0;
	for (int l = 0; l < verySlow.length; l++) {
		if (weapon.contains(verySlow[l])){
	...
	for (int m = 0; m < extremelySlow.length; m++) {
		if (weapon.contains(extremelySlow[m])){
	...
	**/
	...
	public boolean usingHally() {
            String item = c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase();
		if (item.contains("halberd"))
			return true;
		switch(c.playerEquipment[c.playerWeapon]) {
	...
	**/
	...
	public int calculateMeleeMaxHit() {
		...
		/*if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722) {	
                ...
                if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722) {
			double dharokMultiplier = ((1 - ((float)c.constitution / (float)c.maxConstitution)) * 0.95) + 1;
			maxHit *= dharokMultiplier;

		}
		//new hand writen dh effect - mr nick
		/*if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722) {	
	...
	public void handleGmaulPlayer() {
		if (c.playerIndex > 0) {
			...
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), getRequiredDistance())) {
				if (checkReqs()) {
					if (checkSpecAmount(4153) || checkSpecAmount(7668)) {
						int damage = 0;
						if (meleeHitSuccess(calculateMeleeAttack(), calculateMeleeDefence()))
